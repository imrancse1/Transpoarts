...
<form action="{{ route('selectvehicle-update',$selectVehicle->selectvehicles_id) }}" method="POST">
    @csrf
     <div class="row">
    <br>
                   ...
                   <div class="col-sm-9">
                     <select name="input_vehicle_id" class="form-control" id="input_vehicle_id" required="">
                          @if(count($vehicles) > 0)
                           ...
                           <option value="{{$vehicle->input_vehicle_id}}" {{($vehicle->input_vehicle_id == $selectVehicle->input_vehicle_id) ? 'selected' : ''}} >{{$vehicle->vehicle_name}}</option>
                            @endforeach     
                            @endif
                            </select>
